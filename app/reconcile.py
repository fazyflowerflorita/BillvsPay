...
def build_ptl_payroll_output(payroll: pd.DataFrame, matched: pd.DataFrame, rules: Rules) -> pd.DataFrame:
    output = payroll.copy()
    key_cols = ["_employee_id", "_earnings_code", "_period_start", "_period_end"]
    lookup = matched.set_index(key_cols)[["HrsDifference", "AmntDifference", "Comments"]].to_dict(orient="index")
    comments = []
    hour_diffs = []
    amount_diffs = []
    for _, row in output.iterrows():
        key = (row.get("_employee_id", ""), row.get("_earnings_code", ""), row.get("_period_start", ""), row.get("_period_end", ""))
...
def build_ptl_billing_output(billing: pd.DataFrame, payroll: pd.DataFrame, rules: Rules) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    payroll_agg = aggregate_prepared(payroll, "payroll")
    billing_agg = aggregate_prepared(billing, "billing")
    key_cols = ["_employee_id", "_earnings_code", "_period_start", "_period_end"]
    ...
    def comment(row: pd.Series) -> str:
        ...
        if abs(row["HrsDifference"]) > rules.hours_tolerance:
    ...
    for _, row in output.iterrows():
        ...
        hour_diff = result.get("HrsDifference", "")
        amount_diff = result.get("AmntDifference", "")
        hour_diffs.append("" if comment_value in {"Tallied", "Billed but not Paid"} or abs(float(hour_diff or 0)) <= rules.hours_tolerance else hour_diff)
        amount_diffs.append("" if comment_value in {"Tallied", "Billed but not Paid"} or abs(float(amount_diff or 0)) <= rules.amount_tolerance else amount_diff)
    output["Comments"] = comments
    output["HrsDifference"] = hour_diffs
    output["AmntDifference"] = amount_diffs
    ...
    if not payroll_only.empty:
        ...
        for _, row in payroll_only.iterrows():
            ...
            append_row["Week Start"] = row["_period_start"]
            append_row["Week End"] = row["_period_end"]
...
def reconcile(payroll_path: str | Path, billing_path: str | Path, rules_path: str | Path, output_dir: str | Path) -> dict[str, Any]:
    ...
    reconciled = matched[matched["Comments"] == "Tallied"].copy()
    exceptions = matched[matched["Comments"] != "Tallied"].copy()
